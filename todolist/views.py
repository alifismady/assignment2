...
def show_todolist_json(request):
    data_todolist_items = ToDoList.objects.filter(user=request.user)
    return HttpResponse(serializers.serialize("json",data_todolist_items), content_type="application/json")

def create_for_modal(request):
    if request.method == 'POST':
        new = ToDoList.objects.create(user=request.user, date=datetime.datetime.now(),title=request.POST.get('title'),description=request.POST.get('description'),is_finished=False)
